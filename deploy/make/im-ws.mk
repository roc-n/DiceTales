...
build-test:
	...
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o bin/${SERVER_NAME}-${SERVER_TYPE} ./apps/${SERVER_NAME}/${SERVER_TYPE}/${SERVER_TYPE}.go
...
build-local-test: 
	...
	docker cp ./apps/${SERVER_NAME}/${SERVER_TYPE}/etc/${SERVER_TYPE}.yaml ${SERVER_NAME}-${SERVER_TYPE}-test:/${SERVER_NAME}/conf/
...
fresh-local-test: build-test
	...
	docker run -d --name ${SERVER_NAME}-${SERVER_TYPE}-test --network dicetale -p ${PORT}:${PORT} ${APP_NAME_TEST}

local-test:
	@if docker ps -a --format '{{.Names}}' | grep -q "^${SERVER_NAME}-${SERVER_TYPE}-test$$"; then \
		echo "Container exists. Updating binary and restarting..."; \
		$(MAKE) -f deploy/make/${SERVER_NAME}-${SERVER_TYPE}.mk build-local-test; \
	else \
		echo "Container does not exist. Creating and running fresh..."; \
		$(MAKE) -f deploy/make/${SERVER_NAME}-${SERVER_TYPE}.mk fresh-local-test; \
	fi
