CREATE TABLE `post` (
   ...
   `share_count` int(11) DEFAULT '0' COMMENT '分享数(冗余字段)',
...
CREATE TABLE `comment` (
   ...
   `reply_to_uid` bigint(20) unsigned DEFAULT '0' COMMENT '回复目标用户ID',
   `root_id` bigint(20) unsigned DEFAULT '0' COMMENT '根评论ID(二级评论所属的一级ID)',
...
CREATE TABLE `social_interaction` (
   ...
   `action` tinyint(4) NOT NULL COMMENT '动作类型 1:Like 2:Collect 3:Share',
   `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
   PRIMARY KEY (`id`),
